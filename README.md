...
- إصدارات `Windows` و`Linux` الرسومية تحتوي على خيار في شريط النظام باسم `Allow LAN access (0.0.0.0)`

ملاحظة `Linux`:
- في وضع الخادم أو التشغيل بدون واجهة، شغّل الملف التنفيذي مع `--bind 0.0.0.0` إذا كنت تريد الوصول من الشبكة المحلية
...
إذا فعّلت الوصول عبر الشبكة المحلية من جهاز آخر:
- استخدم عنوان `IP` المحلي للجهاز الذي يعمل عليه الخادم بدلًا من `localhost` مثل `http://192.168.1.50`
- واترك المنفذ `8765`
...
- إذا فعّلت الوصول عبر الشبكة المحلية من شريط النظام أو عبر `--bind 0.0.0.0`، اجعل الإضافة تشير إلى عنوان `IP` المحلي للجهاز بدلًا من `localhost`
